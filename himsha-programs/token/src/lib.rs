...
pub fn initialize_mint(mint: Pubkey, authority: Pubkey, decimals: u8, freeze_auth: Option<Pubkey>) -> Instruction {
    Instruction::with_args(
        himsha_runtime::program_ids::token_program(),
        vec![AccountMeta::writable(mint, false)],
        &TokenInstruction::InitializeMint { decimals, mint_authority: authority, freeze_authority: freeze_auth },
    )
}
...
pub fn process(accounts: &mut [AccountInfo], data: &[u8]) -> Result<(), ProgramError> {
    let ix = TokenInstruction::try_from_slice(data)
    ...
    match ix {
        TokenInstruction::InitializeMint { decimals, mint_authority, freeze_authority } => {
            ...
            if mint.is_initialized { return Err(ProgramError::AlreadyInitialized); }
        ...
        TokenInstruction::InitializeAccount => {
            ...
            if !mint.is_initialized { return Err(ProgramError::NotInitialized); }

            let token_acc = &mut accounts[0];
            let mut state: TokenAccountState = token_acc.read_data().unwrap_or_default();
            if state.state != AccountState::Uninitialized { return Err(ProgramError::AlreadyInitialized); }
            state.mint  = mint_key;
            state.owner = owner;
        ...
        TokenInstruction::MintTo { amount } => {
            ...
            if accounts.len() < 3 || !accounts[2].is_signer { return Err(ProgramError::MissingSigner); }
            let mut mint: MintState = accounts[0].read_data()?;
            let mut dest: TokenAccountState = accounts[1].read_data()?;

            if dest.mint != accounts[0].key { return Err(ProgramError::InvalidAccountData); }
            if dest.state == AccountState::Frozen { return Err(ProgramError::Unauthorized); }

            mint.supply = mint.supply.checked_add(amount).ok_or(ProgramError::Overflow)?;
            dest.amount = dest.amount.checked_add(amount).ok_or(ProgramError::Overflow)?;
        ...
        TokenInstruction::Transfer { amount } => {
            ...
            if source.mint != dest.mint { return Err(ProgramError::InvalidAccountData); }
            if source.state == AccountState::Frozen || dest.state == AccountState::Frozen {
                return Err(ProgramError::Unauthorized);
            }
            source.amount = source.amount.checked_sub(amount).ok_or(ProgramError::InsufficientFunds)?;
            dest.amount   = dest.amount.checked_add(amount).ok_or(ProgramError::Overflow)?;
        ...
        TokenInstruction::Burn { amount } => {
            ...
            if token.state == AccountState::Frozen { return Err(ProgramError::Unauthorized); }
            token.amount  = token.amount.checked_sub(amount).ok_or(ProgramError::InsufficientFunds)?;
            mint.supply   = mint.supply.checked_sub(amount).ok_or(ProgramError::InsufficientFunds)?;
        ...
        TokenInstruction::ThawAccount => {
            let mut token: TokenAccountState = accounts[0].read_data()?;
            if token.state != AccountState::Frozen { return Err(ProgramError::InvalidAccountData); }
        ...
        TokenInstruction::CloseAccount => {
            ...
            if token.amount != 0 { return Err(ProgramError::InsufficientFunds); }
...
mod tests {
    ...
    fn token_prog() -> Pubkey { himsha_runtime::program_ids::token_program() }

    fn mint_account(key: &str) -> AccountInfo {
        AccountInfo::new(Pubkey::from_seed(key.as_bytes()), token_prog(), 2_000_000, 128)
    }
    fn token_account(key: &str) -> AccountInfo {
        AccountInfo::new(Pubkey::from_seed(key.as_bytes()), token_prog(), 2_000_000, 256)
    }
    ...
    fn test_initialize_mint_basic() {
        ...
        let ix = borsh::to_vec(&TokenInstruction::InitializeMint {
            decimals: 6, mint_authority: authority, freeze_authority: None,
        }).unwrap();
    ...
    fn test_initialize_mint_with_freeze() {
        ...
        let mut accounts = vec![mint_account("mint")];
        let ix = borsh::to_vec(&TokenInstruction::InitializeMint {
            decimals: 8, mint_authority: auth, freeze_authority: Some(freeze),
        }).unwrap();
        process(&mut accounts, &ix).unwrap();
        let mint: MintState = accounts[0].read_data().unwrap();
    ...
    fn test_initialize_mint_already_initialized() {
        ...
        let mut accounts = vec![mint_account("mint")];
        let ix = borsh::to_vec(&TokenInstruction::InitializeMint {
            decimals: 6, mint_authority: auth, freeze_authority: None,
        }).unwrap();
        process(&mut accounts, &ix).unwrap();
    ...
    fn setup_mint_and_account() -> (Vec<AccountInfo>, Pubkey) {
        ...
        let state = MintState { mint_authority: Some(auth), supply: 0, decimals: 6, is_initialized: true, freeze_authority: None };
        mint_acc.write_data(&state).unwrap();

        let mut token_acc = token_account("token");
        token_acc.key = Pubkey::from_seed(b"token");

        (vec![token_acc, mint_acc, AccountInfo::new(owner, token_prog(), 0, 0)], auth)
    }
    ...
    fn test_initialize_account_uninitialized_mint() {
        ...
        let ix = borsh::to_vec(&TokenInstruction::InitializeAccount).unwrap();
        assert_eq!(process(&mut accounts, &ix), Err(ProgramError::NotInitialized));
    }
    ...
    fn test_transfer_tokens() {
        ...
        let src_state = TokenAccountState {
            mint: mint_key, owner: Pubkey::from_seed(b"alice"),
            amount: 500, state: AccountState::Initialized, ..Default::default()
        };
        let dst_state = TokenAccountState {
            mint: mint_key, owner: Pubkey::from_seed(b"bob"),
            amount: 0, state: AccountState::Initialized, ..Default::default()
        };
    ...
    fn test_transfer_insufficient_balance() {
        ...
        let mut src  = token_account("src");
        let mut dst  = token_account("dst");
        let src_state = TokenAccountState { mint: mint_key, amount: 10, state: AccountState::Initialized, ..Default::default() };
        let dst_state = TokenAccountState { mint: mint_key, amount: 0,  state: AccountState::Initialized, ..Default::default() };
        src.write_data(&src_state).unwrap();
        dst.write_data(&dst_state).unwrap();
    ...
    fn test_transfer_frozen_source() {
        ...
        let mut src  = token_account("src");
        let mut dst  = token_account("dst");
        let src_state = TokenAccountState { mint: mint_key, amount: 500, state: AccountState::Frozen, ..Default::default() };
        let dst_state = TokenAccountState { mint: mint_key, amount: 0,   state: AccountState::Initialized, ..Default::default() };
        src.write_data(&src_state).unwrap();
        dst.write_data(&dst_state).unwrap();
        let owner = AccountInfo::new(Pubkey::default(), token_prog(), 0, 0).as_signer();
    ...
    fn test_burn_reduces_supply() {
        ...
        let mint_state = MintState { supply: 1_000, is_initialized: true, ..Default::default() };
        let tok_state  = TokenAccountState { mint: mint_key, amount: 1_000, state: AccountState::Initialized, ..Default::default() };
    ...
    fn test_burn_more_than_balance() {
        ...
        let mut mint_acc  = mint_account("mint");
        mint_acc.key = mint_key;
        let mint_state = MintState { supply: 100, is_initialized: true, ..Default::default() };
        let tok_state  = TokenAccountState { mint: mint_key, amount: 50, state: AccountState::Initialized, ..Default::default() };
        mint_acc.write_data(&mint_state).unwrap();
        token_acc.write_data(&tok_state).unwrap();
        let owner = AccountInfo::new(Pubkey::default(), token_prog(), 0, 0).as_signer();
        let mut accounts = vec![token_acc, mint_acc, owner];
    ...
    fn test_freeze_and_thaw() {
        ...
        let tok_state = TokenAccountState { mint: mint_key, amount: 100, state: AccountState::Initialized, ..Default::default() };
        token_acc.write_data(&tok_state).unwrap();
    ...
    fn test_thaw_non_frozen_fails() {
        let mint_key = Pubkey::from_seed(b"mint");
        let mut token_acc = token_account("token");
        let tok_state = TokenAccountState { mint: mint_key, amount: 0, state: AccountState::Initialized, ..Default::default() };
        token_acc.write_data(&tok_state).unwrap();
        ...
        assert_eq!(process(&mut accounts, &ix), Err(ProgramError::InvalidAccountData));
    }
    ...
    fn test_approve_and_revoke() {
        ...
        let mut token_acc = token_account("token");
        let tok_state = TokenAccountState { mint: mint_key, amount: 1000, state: AccountState::Initialized, ..Default::default() };
        token_acc.write_data(&tok_state).unwrap();
