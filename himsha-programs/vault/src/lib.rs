...
use himsha_money_market_program::{
    lender_share_value, process as mm_process, LenderPosition, MarketState, MoneyMarketInstruction,
};
use himsha_runtime::{
...
fn program() -> Pubkey {
    himsha_runtime::program_ids::vault_program()
}
...
fn strategy_ix(
    ...
    asset_vault: Pubkey,
    mm_market: Pubkey,
    mm_lender_position: Pubkey,
    mm_borrow_vault: Pubkey,
    manager: Pubkey,
) -> Instruction {
    Instruction::with_args(
        program(),
...
pub fn process(
    ...
    let ix =
        VaultInstruction::try_from_slice(data).map_err(|_| ProgramError::InvalidInstruction)?;
    ...
    match ix {
        VaultInstruction::InitVault {
            ...
            if performance_fee_bps > BPS as u64 {
        ...
        VaultInstruction::Deposit { amount, min_shares } => {
            ...
            let (user_shares, supply_delta) = if vault.total_shares == 0 {
                let u = amount
                    .checked_sub(MIN_SHARES)
                    .ok_or(ProgramError::InsufficientFunds)?;
                (u, amount)
            } else {
                let s = ((amount as u128) * vault.total_shares as u128 / nav_basis.max(1) as u128)
                    as u64;
                (s, s)
            };
        ...
        VaultInstruction::Withdraw { shares, min_assets } => {
            ...
            if idle < assets_out && accounts.len() >= 9 {
                ...
                mm_liquidity(
                    accounts,
                    [6, 7, 2, 8, 0],
                    &MoneyMarketInstruction::RemoveLiquidity { amount: shortfall },
                    timestamp,
                )?;
        ...
        VaultInstruction::Report => {
            ...
            if accounts[1].key != vault.manager {
            ...
            if profit > 0 && vault.performance_fee_bps > 0 && vault.total_shares > 0 {
                ...
                if fee > 0 && (nav as u128) > fee {
                    ...
                    let fee_shares =
                        (fee * vault.total_shares as u128 / (nav as u128 - fee)) as u64;
                    if fee_shares > 0 {
        ...
        VaultInstruction::Deploy { amount } => {
            ...
            if accounts[5].key != vault.manager {
                return Err(ProgramError::Unauthorized);
            }
...
mod tests {
    ...
    fn asset_mint() -> Pubkey {
        Pubkey::from_seed(b"asset-mint")
    }
    fn share_mint_k() -> Pubkey {
        Pubkey::from_seed(b"share-mint")
    }
    fn vault_k() -> Pubkey {
        Pubkey::from_seed(b"vault")
    }
    fn user_k() -> Pubkey {
        Pubkey::from_seed(b"user")
    }
    fn manager_k() -> Pubkey {
        Pubkey::from_seed(b"manager")
    }
    ...
    fn vault_acct(total_assets: u64, total_shares: u64, fee_bps: u64) -> AccountInfo {
        ...
        a.write_data(&VaultState {
            ...
            share_mint: share_mint_k(),
            asset_vault: Pubkey::from_seed(b"asset-vault"),
            manager: manager_k(),
            total_assets,
            lender_shares: 0,
            total_shares,
    ...
    fn shares(a: &AccountInfo) -> u64 {
        a.read_data::<MintState>().unwrap().supply
    }
    ...
    fn dw_accounts(
        ...
        vec![
            ...
            {
                let mut m = share_mint_acct(share_supply);
                m.key = share_mint_k();
                m
            },
    ...
    fn test_init_vault() {
        ...
        let ix = borsh::to_vec(&VaultInstruction::InitVault {
    ...
    fn test_deposit_bootstrap_locks_min_shares() {
        ...
        let ix = borsh::to_vec(&VaultInstruction::Deposit {
            amount: 1_000_000,
            min_shares: 1,
        })
        .unwrap();
    ...
    fn test_withdraw_returns_assets() {
        ...
        let ix = borsh::to_vec(&VaultInstruction::Withdraw {
            shares: 500,
            min_assets: 1,
        })
        .unwrap();
    ...
    fn mm_market_k() -> Pubkey {
        Pubkey::from_seed(b"mm-market")
    }
    fn vault_lender_pos_k() -> Pubkey {
        Pubkey::from_seed(b"vault-lender-pos")
    }
    ...
    fn pos_shares(a: &AccountInfo) -> u64 {
    ...
    fn deploy_accounts(
        ...
        total_shares: u64,
        lender_shares: u64,
        mm_market: AccountInfo,
        mm_lender_pos: AccountInfo,
        mm_borrow_vault_bal: u64,
    ) -> Vec<AccountInfo> {
        vec![
    ...
    fn test_deploy_lends_into_market() {
        ...
        let mut accounts = deploy_accounts(
            1_000,
            1_000,
            1_000,
            0,
            mm_market_acct(0, 0, 0),
            empty_lender_pos(),
            0,
        );
        process(
            &mut accounts,
            &borsh::to_vec(&VaultInstruction::Deploy { amount: 400 }).unwrap(),
            0,
        )
        .unwrap();
    ...
    fn test_deploy_then_undeploy_round_trip() {
        ...
        process(
            &mut accounts,
            &borsh::to_vec(&VaultInstruction::Undeploy { amount: 400 }).unwrap(),
            0,
        )
        .unwrap();
    ...
    fn test_report_reprices_lender_shares_as_yield() {
        ...
        process(
            &mut accounts,
            &borsh::to_vec(&VaultInstruction::Report).unwrap(),
            0,
        )
        .unwrap();
        let v: VaultState = accounts[0].read_data().unwrap();
    ...
    fn test_undeploy_more_than_held_fails() {
        ...
        let mut accounts = deploy_accounts(
            ...
            1_000,
            400,
            mm_market_acct(0, 400, 400),
            vault_lender_pos(400),
            400,
        );
    ...
    fn test_deposit_reprices_off_live_nav() {
        ...
        process(
            &mut accounts,
            &borsh::to_vec(&VaultInstruction::Deposit {
                amount: 1_200,
                min_shares: 1,
            })
            .unwrap(),
