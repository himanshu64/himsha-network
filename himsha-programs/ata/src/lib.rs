...
pub fn get_associated_token_address(owner: &Pubkey, mint: &Pubkey) -> Pubkey {
    let (pda, _) = Pubkey::find_program_address(
        &[owner.as_ref(), himsha_runtime::program_ids::token_program().as_ref(), mint.as_ref()],
...
pub fn process(accounts: &mut [AccountInfo], data: &[u8]) -> Result<(), ProgramError> {
    let ix = AtaInstruction::try_from_slice(data)
        .map_err(|_| ProgramError::InvalidInstruction)?;

    match ix {
        AtaInstruction::Create => {
            if accounts.len() < 4 { return Err(ProgramError::NotEnoughAccounts); }
            ...
            if expected != ata_key { return Err(ProgramError::InvalidAccountData); }

            // If already initialized, succeed idempotently
            if accounts[1].data.len() > 0 { return Ok(()); }
...
mod tests {
    ...
    fn ata_prog() -> Pubkey { himsha_runtime::program_ids::ata_program() }
    ...
    fn mint_account(mint_key: Pubkey, initialized: bool) -> AccountInfo {
        let mut a = AccountInfo::new(mint_key, himsha_runtime::program_ids::token_program(), 0, 128);
        a.write_data(&MintState { is_initialized: initialized, ..Default::default() }).unwrap();
        a
    }
    ...
    fn test_address_is_deterministic_and_unique() {
        ...
        assert_ne!(a, get_associated_token_address(&Pubkey::from_seed(b"owner2"), &mint));
        assert_ne!(a, get_associated_token_address(&owner, &Pubkey::from_seed(b"mint2")));
    }
    ...
    fn test_create_wrong_address_fails() {
        ...
        let data = borsh::to_vec(&AtaInstruction::Create).unwrap();
        assert_eq!(process(&mut accounts, &data), Err(ProgramError::InvalidAccountData));
    }
    ...
    fn test_create_not_enough_accounts() {
        ...
        let data = borsh::to_vec(&AtaInstruction::Create).unwrap();
        assert_eq!(process(&mut accounts, &data), Err(ProgramError::NotEnoughAccounts));
    }
    ...
    fn test_create_uninitialized_mint_fails() {
        ...
        let data = borsh::to_vec(&AtaInstruction::Create).unwrap();
        assert_eq!(process(&mut accounts, &data), Err(ProgramError::NotInitialized));
    }
}
