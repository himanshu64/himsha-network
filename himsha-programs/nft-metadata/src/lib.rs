...
pub fn process(accounts: &mut [AccountInfo], data: &[u8]) -> Result<(), ProgramError> {
    let ix =
        MetadataInstruction::try_from_slice(data).map_err(|_| ProgramError::InvalidInstruction)?;
    ...
    match ix {
        ...
        MetadataInstruction::Update {
            ...
            if accounts[1].key != meta.update_authority {
                return Err(ProgramError::Unauthorized);
            }
            if let Some(n) = name {
                meta.name = n;
            ...
            if let Some(f) = seller_fee_basis_points {
                meta.seller_fee_basis_points = f;
            }
...
mod tests {
    ...
    fn test_create_bad_creator_shares_fails() {
        ...
        let creators = vec![
            Creator {
                address: Pubkey::from_seed(b"c1"),
                share: 40,
                verified: false,
            },
            Creator {
                address: Pubkey::from_seed(b"c2"),
                share: 40,
                verified: false,
            }, // sum 80 ≠ 100
        ];
        assert_eq!(
            do_create(&mut accounts, creators),
    ...
    fn test_create_valid_creator_shares() {
        ...
        assert_eq!(
            accounts[0].read_data::<Metadata>().unwrap().creators.len(),
            2
        );
    }
    ...
    fn test_make_immutable_then_update_fails() {
        ...
        process(
            &mut mk,
            &borsh::to_vec(&MetadataInstruction::MakeImmutable).unwrap(),
        )
        .unwrap();
    ...
    fn test_verify_creator() {
        ...
        process(
            &mut vc,
            &borsh::to_vec(&MetadataInstruction::VerifyCreator).unwrap(),
        )
        .unwrap();
