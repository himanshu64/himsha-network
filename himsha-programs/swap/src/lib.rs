...
pub enum SwapInstruction {
    ...
    Initialize { fee_numerator: u64, fee_denominator: u64 },
    ...
    /// [3] = reserve_a, [4] = reserve_b, [5] = user_lp, [6] = user (signer),
    /// [7] = lp_mint (writable).
    Withdraw { lp_amount: u64, min_a: u64, min_b: u64 },
}

// ---- instruction builders ----

pub fn initialize(pool: Pubkey, token_a_mint: Pubkey, token_b_mint: Pubkey,
    reserve_a: Pubkey, reserve_b: Pubkey, lp_mint: Pubkey, payer: Pubkey,
    fee_num: u64, fee_den: u64) -> Instruction
{
    Instruction::with_args(
        ...
        &SwapInstruction::Initialize { fee_numerator: fee_num, fee_denominator: fee_den },
    )
}

pub fn swap(pool: Pubkey, source: Pubkey, destination: Pubkey,
    reserve_in: Pubkey, reserve_out: Pubkey, user: Pubkey,
    amount_in: u64, min_out: u64) -> Instruction
{
    Instruction::with_args(
        himsha_runtime::program_ids::swap_program(),
        vec![
        ...
        &SwapInstruction::Swap { amount_in, min_amount_out: min_out },
    )
}
...
pub fn process(accounts: &mut [AccountInfo], data: &[u8]) -> Result<(), ProgramError> {
    let ix = SwapInstruction::try_from_slice(data)
        .map_err(|_| ProgramError::InvalidInstruction)?;

    match ix {
        SwapInstruction::Initialize { fee_numerator, fee_denominator } => {
            if fee_denominator == 0 { return Err(ProgramError::InvalidInstruction); }
            if accounts.len() < 7 { return Err(ProgramError::NotEnoughAccounts); }

            let mut pool: PoolState = accounts[0].read_data().unwrap_or_default();
            if pool.is_initialized { return Err(ProgramError::AlreadyInitialized); }
        ...
        SwapInstruction::Swap { amount_in, min_amount_out } => {
            ...
            if !pool.is_initialized { return Err(ProgramError::NotInitialized); }
            ...
            if reserve_in == 0 || reserve_out == 0 { return Err(ProgramError::PoolEmpty); }
            ...
            let fee_amount = amount_in
                ...
                .ok_or(ProgramError::Overflow)?;
            let amount_in_after_fee = amount_in.checked_sub(fee_amount).ok_or(ProgramError::Overflow)?;

            let amount_out = (reserve_out as u128)
                .checked_mul(amount_in_after_fee as u128)
                .ok_or(ProgramError::Overflow)?
                .checked_div((reserve_in as u128).checked_add(amount_in_after_fee as u128).ok_or(ProgramError::Overflow)?)
                .ok_or(ProgramError::Overflow)? as u64;

            if amount_out < min_amount_out { return Err(ProgramError::SlippageExceeded); }
        ...
        SwapInstruction::Deposit { max_a, max_b, min_lp } => {
            // accounts: [0]=pool, [1]=user_a, [2]=user_b, [3]=reserve_a,
            // [4]=reserve_b, [5]=user_lp, [6]=user, [7]=lp_mint.
            if accounts.len() < 8 { return Err(ProgramError::NotEnoughAccounts); }
            ...
            let (deposit_a, deposit_b, user_lp, supply_delta) = if pool.lp_supply == 0 {
                ...
                let user_lp = lp.checked_sub(MINIMUM_LIQUIDITY)
                ...
                let lp_a = max_a.checked_mul(pool.lp_supply).ok_or(ProgramError::Overflow)?
                    .checked_div(reserve_a.max(1)).ok_or(ProgramError::Overflow)?;
                let lp_b = max_b.checked_mul(pool.lp_supply).ok_or(ProgramError::Overflow)?
                    .checked_div(reserve_b.max(1)).ok_or(ProgramError::Overflow)?;
                let lp_out = lp_a.min(lp_b);
                let actual_a = lp_out.checked_mul(reserve_a).ok_or(ProgramError::Overflow)?
                    .checked_div(pool.lp_supply).ok_or(ProgramError::Overflow)?;
                let actual_b = lp_out.checked_mul(reserve_b).ok_or(ProgramError::Overflow)?
                    .checked_div(pool.lp_supply).ok_or(ProgramError::Overflow)?;
                (actual_a, actual_b, lp_out, lp_out)
            };

            if user_lp < min_lp { return Err(ProgramError::SlippageExceeded); }
            ...
            pool.lp_supply = pool.lp_supply.checked_add(supply_delta).ok_or(ProgramError::Overflow)?;
        ...
        SwapInstruction::Withdraw { lp_amount, min_a, min_b } => {
            ...
            let mut pool: PoolState = accounts[0].read_data()?;
            if pool.lp_supply == 0 { return Err(ProgramError::PoolEmpty); }
            ...
            let out_a = (reserve_a as u128)
                .checked_mul(lp_amount as u128).ok_or(ProgramError::Overflow)?
                .checked_div(pool.lp_supply as u128).ok_or(ProgramError::Overflow)? as u64;
            let out_b = (reserve_b as u128)
                .checked_mul(lp_amount as u128).ok_or(ProgramError::Overflow)?
                .checked_div(pool.lp_supply as u128).ok_or(ProgramError::Overflow)? as u64;

            if out_a < min_a || out_b < min_b { return Err(ProgramError::SlippageExceeded); }
            ...
            pool.lp_supply = pool.lp_supply.checked_sub(lp_amount).ok_or(ProgramError::InsufficientFunds)?;
            sync_reserves_ab(accounts, &mut pool)?;
            accounts[0].write_data(&pool)?;
        }
    }
...
mod tests {
    ...
    fn swap_prog() -> Pubkey { himsha_runtime::program_ids::swap_program() }
    ...
    fn lp_accounts(
        reserve_a: u64, reserve_b: u64, lp_supply: u64,
        user_a: u64, user_b: u64, user_lp_bal: u64,
    ) -> Vec<AccountInfo> {
        let mint_a = Pubkey::from_seed(b"mint-a");
        let mint_b = Pubkey::from_seed(b"mint-b");
    ...
    fn test_initialize_pool() {
        let mut accounts = vec![
            pool_account(),
            dummy("mint-a"), dummy("mint-b"),
            dummy("res-a"), dummy("res-b"),
            dummy("lp"), dummy("payer"),
        ];
    ...
    fn test_initialize_zero_fee_denominator() {
        let mut accounts = vec![
            pool_account(),
            dummy("a"), dummy("b"), dummy("ra"), dummy("rb"), dummy("lp"), dummy("p"),
        ];
        let ix = borsh::to_vec(&SwapInstruction::Initialize {
            fee_numerator: 0, fee_denominator: 0, // invalid
        }).unwrap();
        assert_eq!(process(&mut accounts, &ix), Err(ProgramError::InvalidInstruction));
    }
    ...
    fn test_swap_basic() {
        ...
        let ix = borsh::to_vec(&SwapInstruction::Swap {
            amount_in: 100, min_amount_out: 80,
        }).unwrap();
        process(&mut accounts, &ix).unwrap();
    ...
    fn test_swap_slippage_exceeded() {
        ...
        let ix = borsh::to_vec(&SwapInstruction::Swap {
            amount_in: 10, min_amount_out: 999,
        }).unwrap();
        assert_eq!(process(&mut accounts, &ix), Err(ProgramError::SlippageExceeded));
    ...
    fn test_swap_empty_pool() {
        ...
        let ix = borsh::to_vec(&SwapInstruction::Swap {
            amount_in: 100, min_amount_out: 1,
        }).unwrap();
    ...
    fn test_swap_constant_product_invariant() {
        ...
        let ix = borsh::to_vec(&SwapInstruction::Swap {
            amount_in: 1_000, min_amount_out: 1,
        }).unwrap();
        process(&mut accounts, &ix).unwrap();
        let state: PoolState = accounts[0].read_data().unwrap();
    ...
    fn test_deposit_bootstrap() {
        ...
        let ix = borsh::to_vec(&SwapInstruction::Deposit {
            max_a: 1_000_000, max_b: 1_000_000, min_lp: 1,
        }).unwrap();
        process(&mut accounts, &ix).unwrap();
    ...
    fn test_deposit_bootstrap_below_minimum_fails() {
        ...
        let ix = borsh::to_vec(&SwapInstruction::Deposit {
            max_a: 30, max_b: 30, min_lp: 1, // sqrt(900) = 30 < 1000
        }).unwrap();
    ...
    fn test_swap_fee_accrues_to_pool() {
        ...
        let ix = borsh::to_vec(&SwapInstruction::Swap {
            amount_in: 10_000, min_amount_out: 1,
        }).unwrap();
        process(&mut accounts, &ix).unwrap();
    ...
    fn test_withdraw_slippage() {
        ...
        let ix = borsh::to_vec(&SwapInstruction::Withdraw {
            lp_amount: 100, min_a: 500, min_b: 500,
        }).unwrap();
        assert_eq!(process(&mut accounts, &ix), Err(ProgramError::SlippageExceeded));
    }
}
